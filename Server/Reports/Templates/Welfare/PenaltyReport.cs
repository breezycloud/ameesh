...
public class PenaltyReport(PenaltyReportData Model) : IDocument
{
    ...
    void ComposeHeader(IContainer container)
    {
        ...
        container.ShowOnce().Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text($"{StringConverter.ConvertToMonth(Model.Month!.Value)} {Model.Year} Penalty Report");
            });
        });
    }
    ...
    void ComposeTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Header(header =>
            {
                ...
                header.Cell().Element(HeaderCell).AlignCenter().Text("Comment").FontSize(9);
            ...
            // Rows
            foreach (var item in Model.Data.Select((d, i) => new { d, i }))
            {
                table.Cell().Element(Cell).Text((item.i + 1).ToString()).FontSize(9);
                table.Cell().Element(Cell).Text(item.d.Date).FontSize(9);
                table.Cell().Element(Cell).Text(item.d.Staff).FontSize(9);
                table.Cell().Element(Cell).AlignRight().Text($"{item.d.Amount:N2}").FontSize(9);
                table.Cell().Element(Cell).AlignCenter().Text(item.d.Comment ?? "").FontSize(9);
