...
public class ReturnsController(AppDbContext _context) : ControllerBase
{
    ...
    public async Task<IActionResult> PutProduct(Guid id, RefundPayload payload)
    {
        ...
        try
        {
            ...
            await _context.Orders.Where(x => x.Id == payload.Product.OrderId).ExecuteUpdateAsync(s => s.SetProperty(p => p.ModifiedDate, DateTime.UtcNow));
            ...
            {
                await _context.Customers.Where(x => x.Id == customer!.Id).ExecuteUpdateAsync(s => s.SetProperty(p => p.StoreCredit, p => p.StoreCredit + payload!.Product!.Cost).SetProperty(p => p.ModifiedDate, DateTime.UtcNow));
            }
            if (!ProductExists(id))
            {
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        {
            await _context.Customers.Where(x => x.Id == customer.Id).ExecuteUpdateAsync(s => s.SetProperty(p => p.StoreCredit, p => p.StoreCredit - record!.Cost).SetProperty(p => p.ModifiedDate, DateTime.UtcNow));
        }
        return NoContent();
    }
