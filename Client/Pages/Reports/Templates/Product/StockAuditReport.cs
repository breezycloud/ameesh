...
public class StockAuditReport(UserSoldProduct? Model) : IDocument
{
    ...
    void ComposeContent(IContainer container)
    {
        container.PaddingVertical(5).Column(column =>
        {
            ...
            {
                row.RelativeItem().Element(Style).Text("").FontSize(10);
                row.RelativeItem().Element(Style).Text("").FontSize(10);
                row.RelativeItem().Element(Style).AlignCenter().Text($"{Model!.SumTotalQty:N0}").FontSize(10);
                row.RelativeItem().Element(Style).AlignCenter().Text($"{Model!.SumTotalDispensary:N0}").FontSize(10);                
            });
